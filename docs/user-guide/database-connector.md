...
Database AgentPlugin 由管理员配置数据库地址，用户再用自己的数据库账号连接。数据库地址和登录凭据保留在 AgentPlugin Server 内，不会作为工具参数发送给模型。
...
安装器会保留已有配置，并在结束时打印 AgentPlugin 地址和 API Key。

| 内容 | Linux | macOS |
| --- | --- | --- |
| 配置文件 | `/opt/xagent/agent-plugins/database/config.yml` | `~/.local/share/xagent/agent-plugins/database/config.yml` |
| 状态目录 | `/opt/xagent/agent-plugins/database/state` | `~/.local/share/xagent/agent-plugins/database/state` |
| 可执行文件 | `/usr/local/bin/xagent-database-agent-plugin` | `~/.local/bin/xagent-database-agent-plugin` |

Linux 会自动创建并启动 `xagent-database-agent-plugin` systemd 服务；macOS 安装器也会准备并启动托管服务。以下命令仅在需要手工排查时使用：
...
| 字段 | 填写内容 |
| --- | --- |
| AgentPlugin 地址 | 同机安装通常填写 `http://127.0.0.1:19094`；分开部署时填写 xAgent Server 可以访问的地址 |
| API Key | 安装器输出的 API Key，与 `database_agent_plugin.api_key` 一致 |

接入后应显示“在线”和协议 `4.4`。Database AgentPlugin `0.0.4` 及以上会显示独立管理按钮。没有按钮时，先升级 AgentPlugin，并确认 API Key 不为空。
...
![Database AgentPlugin 数据库资源列表](/img/user-guide/connectors/database-management-zh.webp)

点击“添加数据库”：

![Database AgentPlugin 添加数据库对话框](/img/user-guide/connectors/database-add-resource-zh.webp)
...
一个用户可以连接多个数据库。再次连接同一资源会替换旧凭据。SQL 是否可执行由数据库账号本身的权限决定，AgentPlugin 不会提升权限。
...
Database AgentPlugin 提供 `db_list`、`db_getinfo` 和 `db_execute_sql`。生产环境应使用最小权限数据库账号；只读场景直接使用数据库原生的只读账号或角色。
...
AgentPlugin 不监听手工文件变化。直接修改 `config.yml` 后需要重启：
...
| 现象 | 处理方法 |
| --- | --- |
| AgentPlugin 离线 | 检查服务、`19094` 监听、网络和 API Key |
| 没有管理按钮 | 升级到 `0.0.4` 以上，并确认使用配置文件启动且 API Key 非空 |
| 新资源没有出现在“插件连接” | 确认保存成功、AgentPlugin 在线，然后刷新 AgentPlugin Card 和用户页面 |
| 用户认证失败 | 从 AgentPlugin Server 检查数据库网络、数据库名、用户名和密码 |
| SQL 被拒绝 | 查看数据库返回的权限或 SQL 错误，调整数据库账号或 SQL |
| 手工修改配置后没有变化 | 重启 AgentPlugin；只有管理页面保存的资源会立即应用 |
