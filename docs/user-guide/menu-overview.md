---
title: 功能导览与菜单入口
description: 了解 v0.0.5.beta 控制台的信息架构、简洁与高级模式，以及普通用户和管理员的主要菜单入口。
status: stable
updated: 2026-07-27
---
...
`v0.0.5.beta` 重新整理了控制台的品牌、排版密度、页面层级和响应式布局，并统一了列表、对话框与侧边抽屉的交互。会话页也重新组织了消息时间线、附件、Tool 调用状态和工作区入口，在窄屏与移动端上更容易连续使用。

侧边栏现在按职责分组，不再把偏好设置、主题设置和邮箱管理分别作为独立菜单。账号信息、界面主题和显示密度统一放在“账号管理”中；多套界面配色已取消。

> 改版前的截图不再放在本文中，以免页面结构和按钮位置误导当前版本用户。请以 `v0.0.5.beta` 实际界面为准。
...
企业账号入口当前仍隐藏，不作为 `v0.0.5.beta` 的可用功能说明。
