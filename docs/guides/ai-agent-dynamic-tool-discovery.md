...
工具、MCP、 AgentPlugin 和 Skill 会持续变化。如果每个会话都固定携带全部能力，新增、下线或调整一项能力都会影响更大的上下文和测试范围。
...
## MCP 和 AgentPlugin 能力如何进入发现范围

Tool 不只来自系统内置能力，也可能来自个人 MCP、管理员配置的 MCP 或已经完成授权的 AgentPlugin 。
...
5. 检查当前用户 AgentPlugin 是否在线、发送工具是否可用以及是否需要审批。
6. 获得允许后发送摘要，并根据工具返回结果确认是否成功送达。

整个过程中不需要在会话开始时加载所有表格、报告、MCP 和 AgentPlugin 能力。
