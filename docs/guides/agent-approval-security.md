---
title: "AI Agent 安全风险：xAgent 审批与安全控制如何工作"
description: 了解 xAgent 当前如何处理会触发工具调用的 AI Agent 安全风险：识别文件、网络、数据、会话和 Skill 操作，并在特定高风险动作前暂停、审批和恢复。
image: /img/share/zh/xagent-security.png
status: beta
updated: 2026-08-06
---

# AI Agent 安全风险：xAgent 审批与安全控制如何工作

AI Agent 不只是生成文字，还可能读取或删除文件、执行命令、访问网络、发送数据、修改外部系统，或者发布团队可见的 Skill。审批的作用不是在任务开始前审批整段对话，而是在任务执行到具体风险动作时暂停，让用户确认后再继续。

## 当前能处理哪些 AI Agent 安全风险

xAgent 当前审批机制处理的是 Agent 准备执行的具体操作，而不是对所有安全风险给出完整防护。下表说明当前页面能如实承接的范围：

| 风险动作 | 当前控制方式 | 不能据此声称的能力 |
| --- | --- | --- |
| 删除或写入工作区文件 | 根据 Tool 和参数识别操作事实，并由审批策略决定放行、拒绝或等待确认 | 不代表所有文件、主机或外部系统都有相同保护 |
| 向外部系统发送个人数据 | 识别个人数据传输操作；默认策略可要求单次审批 | 不替代外部系统的数据权限、数据处理协议或网络出口控制 |
| 执行网络请求或命令 | 识别网络和进程执行操作；管理员可按团队要求配置更严格规则 | 不代表已覆盖 prompt injection、供应链或所有恶意输入风险 |
| 激活 Skill 草稿或提交公共 Skill | 识别 Skill 发布相关操作，并按策略暂停或拒绝 | 不替代对 Skill 内容、依赖或第三方来源的审查 |

Prompt injection、过度授权、模型与 Provider 数据边界、第三方 Skill/MCP 供应链和安全测试都需要各自的设计与验证。当前审批页只能说明其中的动作审批边界，不能作为完整 AI Agent 安全框架的替代品。
