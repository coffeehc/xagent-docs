...
`v0.0.3.beta` 的默认策略会直接允许主会话创建子会话，也允许删除当前会话产物目录下的文件。以下操作默认需要单次审批：
...
用户可以在 Web 会话中查看审批内容并决定是否继续。`v0.0.3.beta` 也支持把来自 IM 连接器的任务审批通知发回原连接通道：

- 当前任务输入来自 IM 连接器，并且原连接通道仍然在线、可发送消息时，xAgent 会尝试把审批通知发回该通道。
- 通知包含审批编号、动作摘要和相关工具信息。
- 用户可以按通知显示的格式回复，例如 `#审批号 同意` 或 `#审批号 不同意`。
- 第一个有效回复完成审批后，后续对同一审批编号的回复不会再次改变结果。

当前版本不会把 Web 发起任务的审批广播到用户全部 IM 通道。是否能够从 IM 审批，取决于任务是否来自该连接器、连接器是否在线、用户是否完成认证，以及原通道是否提供可用的消息发送工具。
