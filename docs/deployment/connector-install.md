...
Connector 是连接器服务，用于把微信、Telegram、邮件、企业系统或第三方入口接入 xAgent。和 MCP 不同，连接器可以主动接收外部消息，并把消息或事件投递到 xAgent 的目标会话中。
...
- 通过微信、Telegram、邮件或其他 IM 工具远程下达任务。
...
不同 Connector 的二进制名称、参数和依赖可能不同。当前发布包已包含微信和 Telegram 连接器，实际部署时以对应连接器说明为准。

| 连接器 | 适用场景 | 用户侧认证方式 |
| --- | --- | --- |
| 微信 Connector | 在微信中接收或发送消息 | 按页面提示扫码或授权 |
| Telegram Connector | 通过 Telegram Bot 接收或发送消息 | 绑定自己的 `bot_token` 和目标 `chat_id` |
...
## Telegram Connector 启动

Telegram Connector 使用 Telegram Bot API。连接器服务本身不要求把 Bot Token 写进命令行或 `config.yml`；每位用户在 xAgent 的“我的连接”中绑定自己的 `bot_token` 与目标 `chat_id`，连接器把 Token 保存在本地状态目录中。

常用配置项：

| 配置项 | 命令行参数 | 默认值 | 说明 |
| --- | --- | --- | --- |
| `telegram_connector.addr` | `--addr` | `127.0.0.1:19091` | Connector HTTP 监听地址 |
| `telegram_connector.api_key` | `--api-key` | `test-api` | xAgent 访问 Connector 接口时使用的鉴权密钥 |
| `telegram_connector.state_dir` | `--state-dir` | `~/.xagent/connectors/im-telegram` | 保存用户绑定、Bot Token 和消息状态的目录 |
| `telegram_connector.api_base_url` | `--telegram-api-base-url` | Telegram 官方 API 地址 | Telegram Bot API 服务地址；仅在使用兼容网关时调整 |
...
首次使用时，用户在“我的连接”中选择 Telegram Connector，按认证表单填入自己的 `bot_token` 和目标 `chat_id`。绑定私聊前，用户需要先向该 Bot 发送 `/start` 或任意消息，Telegram Bot API 才能读取该私聊。
...
微信与 Telegram Connector 默认提供的 Connector Card 入口分别为：

```text
http://连接器地址:19090/connector-card.json  # 微信
http://连接器地址:19091/connector-card.json  # Telegram
```
...
- Connector 状态目录，例如 `/var/lib/xagent-connectors/wechat/state` 或 `/var/lib/xagent-connectors/telegram/state`
- systemd service 文件
- 反向代理配置
