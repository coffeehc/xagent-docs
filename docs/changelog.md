...
- 历史 v2 `free` 或 v1 `temporary` 证书不再接受；启动时会删除对应证书文件并进入无证书免费版。
...
- 后台提取会先判断内容应进入长期记忆、仅保留在当前会话、拒绝还是需要复核。问句、假设、未接受方案、临时任务状态和敏感信息不会直接写入长期记忆。
...
- 会话压缩期间，信息面板会持续展示当前压缩状态，不再被较早的 Tool、模型或连接提示覆盖。
- 同一轮中的多个 Tool 调用可以在受控范围内并发执行。即使其中一个调用正在等待审批或继续，已经完成的调用结果也会稳定保留。
...
- 修复 macOS 上 Python 依赖目录与沙箱规则冲突的问题；修复 Linux 只读上传文件挂载和残留进程清理，进一步收紧本地命令执行边界。
- 侧栏版本号改为简洁的三段版本；管理员可以点击版本号立即检查更新，并在发现新版本后继续一键升级。
...
- macOS 和 Linux 会迁移旧执行运行时目录，保留用户 Python 依赖并删除已经失效的临时执行状态。
